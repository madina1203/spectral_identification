...
class FixedHoldoutPUCallback(Callback):
    ...
    def on_validation_epoch_end(self, trainer, pl_module):
        ...
        # The val_true_labels numpy array should be 1D due to all_labels tensor reshape.
        # So, this line can remain commented or be removed.
        # if val_true_labels.ndim == 2 and val_true_labels.shape[1] == 1: val_true_labels = val_true_labels.reshape(-1)
        ...
        if len(val_probs_s1) != len(val_true_labels):
            ...
            # Details about original tensor shapes might be less relevant here as it's post local cat.
            # print(f"[Rank {rank}] Details: all_logits.shape was {all_logits.shape}, all_labels.shape was {all_labels.shape}")
            return
...
def calculate_pu_metrics(probabilities, true_labels, labeled_pos_indices):
        ...
        try:
            ...
            if std_neg < 1e-6 or std_pos < 1e-6:
                ...
                if len(fpr_final) > 1:
                    ...
                    print(
                        "Warning (v2 script): Not enough valid points for AUC GMM. Defaulting to 0.5."); auroc_gmm_val = 0.5
    ...
    if len(labeled_pos_indices) == 0:
        ...
        if len(labeled_pos_ranks) > 0:
            ...
            if n_pos > 1:
                x_vals = np.arange(1, n_pos + 1) / n_pos; print(f"X-vals (AUPRC): {x_vals}"); auprc_val = np.trapz(
        ...
        if len(labeled_pos_ranks) > 0:
            epr_val = np.mean(labeled_pos_ranks > threshold_epr); print(f"EPR: {epr_val:.4f}")
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--hold_out_ratio", type=float, default=0.3, help="Hold-out ratio for PU learning")

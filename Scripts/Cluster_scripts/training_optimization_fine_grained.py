...
def train_model_smac(config, seed=None, budget=None):
    ...
    # Optimizer name is now always AdamW
    optimizer_name = "AdamW"
    # FIXED: n_layers is now static
    n_layers = 2
    ...
    csv_logger = CSVLogger(save_dir="logs", name="smac_optimization_fine_grained_27_06")
...
def run_smac_optimization():
    ...
    cs.add_hyperparameter(Categorical("hidden_fc1", [32, 48, 64, 96]))

    cs.add_hyperparameter(Categorical("instrument_embedding_dim", [16, 32]))


    scenario = Scenario(
        configspace=cs,
        name="optimization_cluster_fine_grained_27_06",
        n_trials=25,
        objectives=["val_loss"],
        deterministic=True,
        min_budget=8,

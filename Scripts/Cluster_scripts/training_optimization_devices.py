...
def train_model_smac(config, seed=None, budget=None):
    ...
    csv_logger = CSVLogger(save_dir="logs", name="smac_optimization_30_04_devices")
    ...
    if torch.distributed.is_initialized():  # safety check
        trainer.strategy.barrier()  # wait both ranks
        torch.distributed.destroy_process_group()  # free the port
    return val_loss
...
def run_smac_optimization():
    ...
    scenario = Scenario(
        configspace=cs,
        name="optimization_single_worker_30_04",
        n_trials=72,
        objectives=["val_loss"],
        deterministic=True,
        min_budget=6,
        max_budget=50,
        n_workers=1  # specifying one worker for smac
    )

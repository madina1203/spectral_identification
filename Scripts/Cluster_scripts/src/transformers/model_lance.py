...
class SimpleSpectraTransformer(pl.LightningModule):
    ...
    def forward(self, batch):
        ...
        instrument_settings = batch["instrument_settings"]  # Shape: (batch_size, features)
        ...
        if spectra_emb.dim() != instrument_emb.dim():
            # If instrument_emb has 3 dimensions but spectra_emb has 2, squeeze instrument_emb
            if instrument_emb.dim() > spectra_emb.dim():
                instrument_emb = instrument_emb.squeeze(1)
            # If spectra_emb has 3 dimensions but instrument_emb has 2, unsqueeze instrument_emb
            elif spectra_emb.dim() > instrument_emb.dim():
                instrument_emb = instrument_emb.unsqueeze(1)
    ...
    def training_step(self, batch, batch_idx):
        ...
        step_end = time.time()
        print(f"[PROFILE] Training step {batch_idx}: {step_end - step_start:.4f} seconds")
    ...
    def validation_step(self, batch, batch_idx):
        ...
        loss, preds, targets = self.step(batch)
        step_end = time.time()
        print(f"[PROFILE] Validation step {batch_idx}: {step_end - step_start:.4f} seconds")

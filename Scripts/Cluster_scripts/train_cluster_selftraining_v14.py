...
class ImprovedPyTorchSklearnWrapper(BaseEstimator, ClassifierMixin):
    ...
    def __init__(self, model=None, dataset=None, d_model=64, n_layers=2,
        ...
        # Persist previous_labels across self-training iterations
...
def plot_probability_distribution(probabilities, gmm, epoch, log_dir):
    ...
    try:
        ...
        plot_dir = os.path.join(log_dir, 'probability_distributions_18_07_v3')
...
def calculate_pu_metrics(probabilities, true_labels, labeled_pos_indices, log_dir=None, iteration=None, epoch=None):
    ...
    print("\n-- Percentile Rank Calculation (v2 script) --")
    if len(labeled_pos_indices) == 0:
        print("Warning (v2 script): No valid labeled positive for AUPR.")
        area_under_percentile_ranks = 0.0
    else:
        if len(probabilities) > 1 and np.max(probabilities) != np.min(probabilities):  # Avoid division by zero for uniform probabilities
    ...
    return {'val_auroc_gmm': float(auroc_gmm_val), 'val_area_under_percentile_ranks': float(area_under_percentile_ranks)}
...
class SelfTrainingMetricsCallback(pl.Callback):
    def __init__(self, wrapper_instance, val_indices, combined_dataset, log_dir, self_training_iteration,
                 val_true_labels_pu, train_indices=None):
        super().__init__()
        ...
        self.val_true_labels_pu = val_true_labels_pu
        self.train_indices = train_indices  # NEW: store training indices

    def on_fit_start(self, trainer, pl_module):
        print(f"[DEBUG] on_fit_start: previous_labels is None? {self.wrapper_instance.previous_labels is None}")
        if hasattr(self.wrapper_instance, '_self_training_iteration'):
            print(f"[DEBUG] on_fit_start: iteration = {self.wrapper_instance._self_training_iteration}")
        print(f"[DEBUG] on_fit_start: train_indices is None? {self.train_indices is None}")
        if self.train_indices is not None:
            print(f"[DEBUG] on_fit_start: train_indices length = {len(self.train_indices)}")
        ...
        if hasattr(self.wrapper_instance, '_self_training_estimator'):
            ...
            if hasattr(estimator, 'transduction_'):
                ...
                if self.wrapper_instance.previous_labels is not None and iteration > 0:
                    prev_unlabeled = (self.wrapper_instance.previous_labels == -1)
                    now_pos = (self.wrapper_instance.previous_labels == -1) & (current_labels == 1)
                    now_neg = (self.wrapper_instance.previous_labels == -1) & (current_labels == 0)
                    still_unlabeled = (self.wrapper_instance.previous_labels == -1) & (current_labels == -1)

                    # Efficiently get probabilities for all training samples
                    if self.train_indices is not None:
                        X_train_indices = np.array(self.train_indices).reshape(-1, 1)
                        train_probabilities = self.wrapper_instance.predict_proba(X_train_indices)[:, 1]
                        dfs = []
                        for mask, group in zip([now_pos, now_neg, still_unlabeled],
                                               ['now_positive', 'now_negative', 'still_unlabeled']):
                            probs = train_probabilities[mask]
                            df = pd.DataFrame({'iteration': [iteration-1]*len(probs), 'group': [group]*len(probs), 'probability': probs})
                            dfs.append(df)
                        if dfs:
                            result_df = pd.concat(dfs, ignore_index=True)
                            print(f"[DEBUG] on_fit_start: result_df shape = {result_df.shape}")
                            out_path = os.path.join(self.log_dir, 'train_probabilities_across_iterations.csv')

                            header = not os.path.exists(out_path)
                            result_df.to_csv(out_path, mode='a', header=header, index=False)

                    newly_labeled_mask = (self.wrapper_instance.previous_labels == -1) & (current_labels != -1)
                    total_newly_labeled = np.sum(newly_labeled_mask)
                    newly_labeled_pos = np.sum((self.wrapper_instance.previous_labels == -1) & (current_labels == 1.0))
                    newly_labeled_neg = np.sum((self.wrapper_instance.previous_labels == -1) & (current_labels == 0.0))
                ...
                self.wrapper_instance.previous_labels = current_labels.copy()
    ...
    def on_validation_epoch_end(self, trainer, pl_module):
        ...
        # --- NEW: Save probability distributions for plotting ---
        # 1. Known positives
        mask_known_positives = (val_true_labels_original_format == 1)
        probs_known_positives = val_probabilities[mask_known_positives]
        # 2. Negatives labeled as positives
        mask_neg_labeled_pos = (val_true_labels_original_format == 0) & (val_predictions == 1)
        probs_neg_labeled_pos = val_probabilities[mask_neg_labeled_pos]
        # 3. Negatives labeled as negatives
        mask_neg_labeled_neg = (val_true_labels_original_format == 0) & (val_predictions == 0)
        probs_neg_labeled_neg = val_probabilities[mask_neg_labeled_neg]

        # Save to CSVs
        if current_epoch == (self.wrapper_instance.epochs - 1):
            epoch_label = f"iter_{self.self_training_iteration}_epoch_{current_epoch}"
            if self.log_dir is not None:
                os.makedirs(self.log_dir, exist_ok=True)
                pd.DataFrame({'probability': probs_known_positives}).to_csv(
                    os.path.join(self.log_dir, f"probs_known_positives_{epoch_label}.csv"), index=False)
                pd.DataFrame({'probability': probs_neg_labeled_pos}).to_csv(
                    os.path.join(self.log_dir, f"probs_negatives_labeled_as_positives_{epoch_label}.csv"), index=False)
                pd.DataFrame({'probability': probs_neg_labeled_neg}).to_csv(
                    os.path.join(self.log_dir, f"probs_negatives_labeled_as_negatives_{epoch_label}.csv"), index=False)
        # Get predictions and calculate metrics
...
def main(args):
    ...
    pytorch_model = ImprovedPyTorchSklearnWrapper(
        ...
        log_dir=log_dir,  # <-- Use the full log_dir with run name
...
if __name__ == "__main__":
    ...
    parser.add_argument("--run_name", type=str, default="self_training_18_07_v3", help="Name of the run")

...
def train_model_smac(config, seed=None, budget=None):
    ...
    hidden_fc1 = d_model if hidden_fc1_choice == "d_model" else 2 * d_model
    ...
    early_stopping = EarlyStopping(monitor="val_loss", patience=2, mode="min", verbose=True)
    model_checkpoint = ModelCheckpoint(monitor="val_loss", mode="min", save_top_k=1, verbose=True)

    csv_logger = CSVLogger(save_dir="logs", name="smac_optimization", version="version_1")
...
def run_smac_optimization():
    ...
    cs.add_hyperparameter(Categorical("d_model", [64, 128, 256, 512]))
    cs.add_hyperparameter(Categorical("learning_rate", [1e-4, 1e-3]))
    cs.add_hyperparameter(Categorical("optimizer_name", ["Adam", "AdamW"]))
    cs.add_hyperparameter(Categorical("hidden_fc1_choice", ["d_model", "2xd_model"]))
    cs.add_hyperparameter(Categorical("n_layers", [2, 3, 4, 5, 6]))
    scenario = Scenario(
        configspace=cs,
        n_trials=10,
        objectives=["val_loss"],

...
# The collate_fn has been moved to SimpleMassSpecDataset as a static method
...
def prepare_dataloaders(file_paths, batch_size, val_split, num_workers):
    ...
    train_loader = DataLoader(
        ...
        collate_fn=SimpleMassSpecDataset.collate_fn,
...
def train_model_smac(config, seed=None, budget=None):
    # Printing the config info
    # Use DASK worker ID to assign a GPU
    try:
        from dask.distributed import get_worker
        worker = get_worker()
        name = worker.name  # e.g., 'worker-0'
        worker_id = int(name.split('-')[-1])
    except Exception:
        print("[Fallback] Couldn't get Dask worker. Defaulting to GPU 0.")
        worker_id = 0
    ...
    if torch.distributed.is_initialized():
        ...
        rank = 0
    print(f"[Rank {rank}] Config before model creation: {config}")
    ...
    # Printing the number of parameters by layers
    for name, module in model.named_children():
        params = sum(p.numel() for p in module.parameters())
        print(f"[Rank {rank}] {name}: {params} parameters")
    ...
    csv_logger = CSVLogger(save_dir="logs", name="smac_optimization_24_04_workers")
...
def run_smac_optimization():
    ...
    cs.add_hyperparameter(UniformFloatHyperparameter("learning_rate", 1e-5, 1e-2, log=True))
    ...
    scenario = Scenario(
        configspace=cs,
        name="optimization_two_workers_24_04",
        n_trials=50,
        objectives=["val_loss"],
        deterministic=False,
        min_budget=6,
        max_budget=50,
        n_workers=2  # specifying 2 workers for smac
    )
